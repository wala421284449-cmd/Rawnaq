...
@section('title', 'تعديل صورة المعرض | متجر رونق')
@section('main-title', 'إدارة معرض المتاجر')
@section('sub-title', 'تعديل بيانات وصورة المعرض التابعة للمتجر ضمن منصة متجر رونق')
                    ...
                    <div class="card custom-form-card border-0">
                        ...
                        <div class="form-header d-flex justify-content-between align-items-center">
                            <div class="d-flex align-items-center gap-3">
                                ...
                                <div>
                                    <h5 class="fw-bold mb-1" style="color: var(--rawnaq-dark-title);">تعديل صورة المعرض</h5>
                                    <small class="text-muted">تحديث بيانات أو استبدال صورة المعرض الخاصة بالمتجر</small>
                                </div>
                            </div>
                            <a href="{{ route('media-galleries.index') }}" class="btn-return-custom d-flex align-items-center gap-2">
                        ...
                        <!-- Form -->
                        <form id="update_gallery_form" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="p-4">
                                    ...
                                    <div class="row g-3">
                                        ...
                                        <div class="col-md-12">
                                            <label for="stores_id" class="custom-label">
                                                <span>المتجر التابع له الصورة</span>
                                            ...
                                            <div class="input-group input-group-custom">
                                                ...
                                                <select name="stores_id" id="stores_id" class="form-select" required>
                                                    ...
                                                    @foreach ($stores ?? [] as $store)
                                                        <option value="{{ $store->id }}" {{ (old('stores_id', $mediaGallery->stores_id) == $store->id) ? 'selected' : '' }}>
                                                            {{ $store->name }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <div class="input-group input-group-custom">
                                                ...
                                                <input type="text" name="title" id="title" class="form-control"
                                                    value="{{ old('title', $mediaGallery->title) }}" placeholder="وصف الصورة...">
                                            </div>
                                        </div>
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <div class="input-group input-group-custom">
                                                ...
                                                <input type="text" name="media_type" id="media_type" class="form-control"
                                                    value="{{ old('media_type', $mediaGallery->media_type) }}">
                                            </div>
                                        </div>
                                    </div>
                                </div>

                                <!-- Section 2: الصورة الحالية واستبدالها -->
                                <div class="section-box">
                                    <div class="section-badge">
                                    ...
                                    <div class="row g-3 align-items-center">
                                        ...
                                        @if($mediaGallery->file_path)
                                            <div class="col-md-3 text-center">
                                                <label class="custom-label justify-content-center mb-2"><span>الصورة الحالية</span></label>
                                                <img src="{{ asset($mediaGallery->file_path) }}" alt="Gallery Image" class="current-img-preview">
                                            </div>
                                        @endif

                                        <!-- New Image File Input -->
                                        <div class="{{ $mediaGallery->file_path ? 'col-md-9' : 'col-md-12' }}">
                                            <label for="image" class="custom-label">
                                                <span>استبدال الصورة (اختياري)</span>
                                                <span class="badge-opt">اتركها فارغة إن لم ترد التغيير</span>
                                            </label>
                                            <div class="input-group input-group-custom">
                                                <span class="input-group-text"><i class="bi bi-upload"></i></span>
                                                <input type="file" name="image" id="image" class="form-control" accept="image/*">
                                            </div>
                                            <small class="text-muted mt-1 d-block">سيتم الاحتفاظ بالصورة القديمة تلقائياً في حال عدم اختيار صورة جديدة.</small>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="form-footer d-flex justify-content-end align-items-center gap-3">
                                ...
                                <button type="button" onclick="performUpdateGallery({{ $mediaGallery->id }})"
                                    class="btn-save-custom d-flex align-items-center gap-2">
                                    <i class="bi bi-check2-circle fs-5"></i>
                                    <span>تحديث بيانات الصورة</span>
                                </button>
                            </div>
                        </form>
                    </div>
...
@section('scripts')
    <script>
        function performUpdateGallery(id) {
            let formData = new FormData();
            formData.append('_method', 'PUT'); // إعلام لارافيل بأن الطلب تحديث
            ...
            // جلب الصورة الجديدة في حال تم اختيارها للاستبدال
            let imageInput = document.getElementById('image');
            if (imageInput.files[0]) {
                formData.append('image', imageInput.files[0]);
            }

            axios.post('/cms/admin/media-galleries/' + id, formData, {
                headers: {
            ...
            .catch(function(error) {
                let message = 'حدث خطأ أثناء تحديث بيانات الصورة';
