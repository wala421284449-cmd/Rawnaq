...
@section('title', 'تعديل الطلب | متجر رونق')
@section('main-title', 'الطلبات والحجوزات')
@section('sub-title', 'تحديث بيانات الطلب أو الحجز المحدد')
                    ...
                    <div class="card custom-form-card border-0">
                        <div class="form-header d-flex justify-content-between align-items-center">
                            <div class="d-flex align-items-center gap-3">
                                ...
                                <div>
                                    <h5 class="fw-bold mb-1" style="color: var(--rawnaq-dark-title);">تعديل الطلب رقم:
                                        #{{ $order->id }}</h5>
                                    <small class="text-muted">تحديث بيانات العميل والحالة والمبلغ</small>
                                </div>
                            </div>
                        ...
                        <form id="update_order_form">
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <div class="input-group input-group-custom">
                                                ...
                                                <input type="text" name="customer_phone" id="customer_phone"
                                                    class="form-control" value="{{ $order->customer_phone }}" required>
                                            </div>
                                        </div>
                                        <div class="col-md-6">
                                                ...
                                                <select name="stores_id" id="stores_id" class="form-select" required>
                                                    @foreach ($stores ?? [] as $store)
                                                        <option value="{{ $store->id }}"
                                                            {{ $order->stores_id == $store->id ? 'selected' : '' }}>
                                                            {{ $store->name }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                        ...
                                        <div class="col-md-4">
                                            ...
                                            <div class="input-group input-group-custom">
                                                ...
                                                <input type="text" name="total_amount" id="total_amount"
                                                    class="form-control" value="{{ $order->total_amount }}" required>
                                            </div>
                                        </div>
                                        <div class="col-md-4">
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <div class="input-group input-group-custom">
                                                ...
                                                <input type="date" name="booking_date" id="booking_date"
                                                    class="form-control" value="{{ $order->booking_date }}">
                                            </div>
                                        </div>
                                        <div class="col-md-6">
                                            <label class="custom-label"><span>bookings_or_orderscol</span></label>
                                            <div class="input-group input-group-custom">
                                                <span class="input-group-text"><i class="bi bi-file-text"></i></span>
                                                <input type="text" name="bookings_or_orderscol"
                                                    id="bookings_or_orderscol" class="form-control"
                                                    value="{{ $order->bookings_or_orderscol }}">
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="form-footer d-flex justify-content-end align-items-center gap-3">
                                ...
                                <button type="button" onclick="performUpdateOrder({{ $order->id }})"
                                    class="btn-save-custom">تحديث الطلب</button>
                            </div>
                        </form>
                    </div>
                </div>
...
@section('scripts')
    <script>
        function performUpdateOrder(id) {
            let formData = {
            ...
            axios.put('/cms/admin/orders/' + id, formData)
